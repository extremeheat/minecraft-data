...
async function updateManifestPC (resyncEverything) {
  ...
  async function handleVersion (latestVersion, latestVersionData) {
    const latestVersionIsSnapshot = resyncEverything || latestVersionData.type !== 'release'
...
async function addEntryFor (releaseVersion, releaseData) {
  ...
  fs.rmSync('version.json', { force: true })
  ...
  const versionJson = JSON.parse(fs.readFileSync('./version.json'))
  ...
  protocolVersions.pc = updatedProtocolVersions
